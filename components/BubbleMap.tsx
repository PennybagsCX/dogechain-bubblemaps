...
import { 
  Move, MousePointer2, ZoomIn, ZoomOut, HelpCircle,
  Crosshair, Camera, Pause, Play, RotateCcw, EyeOff, Eye, ChevronDown, ChevronUp,
  Settings, Sliders, X
} from 'lucide-react';
...
export const BubbleMap: React.FC<BubbleMapProps> = ({
  ...
  const [dimensions, setDimensions] = useState({ width: initialWidth || 800, height: initialHeight || 600 });
  ...
  useEffect(() => {
      const handleKeyNav = (e: KeyboardEvent) => {
        ...
        if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
            ...
            newIndex = (focusedIndex === null) ? wallets.length - 1 : (focusedIndex - 1 + wallets.length) % wallets.length;
            setFocusedIndex(newIndex);
            handleSelectNode(getWalletAt(newIndex));
        }
      };
  ...
  useEffect(() => {
    ...
    const foundUser = userAddress ? wallets.some(w => w.address.toLowerCase() === userAddress.toLowerCase()) : false;
    ...
    // --- DATA PREPARATION ---
    const nodes: NodeDatum[] = wallets.map((w, idx) => ({ ...w, x: width / 2, y: height / 2, rank: idx + 1 })) as NodeDatum[];
    ...
    const simulation = d3.forceSimulation(nodes)
        .velocityDecay(0.2)
        .alphaDecay(0.005)
        .force("link", d3.forceLink<NodeDatum, LinkDatum>(linksCopy).id((d: NodeDatum) => d.id).distance(80).strength(0.1)) 
        .force("charge", d3.forceManyBody<NodeDatum>().strength((d: NodeDatum) => -d.r * 3 - 15)) 
        .force("collide", d3.forceCollide<NodeDatum>().radius((d: NodeDatum) => d.r + 4).strength(0.8).iterations(3)) 
    ...
    nodeSelection
      ...
      .on("mouseover", function(_event: any, d: NodeDatum) {
        ...
        linkSelection
          .filter((l: LinkDatum) => (l.source as NodeDatum).id === hoveredId || (l.target as NodeDatum).id === hoveredId)
    ...
    simulation.on("tick", () => {
        ...
        rankSelection.attr("x", (d: NodeDatum) => d.x).attr("y", (d: NodeDatum) => d.y - Math.min(d.r * 0.4, 12));
  ...
  }, [wallets, links, dimensions, onWalletClick, assetType, userAddress]); // Dependencies
  ...
  const handleLocateUser = () => {
      if (!simulationRef.current || !svgRef.current || !zoomBehaviorRef.current || !userAddress) return;
      ...
      if (userNode) {
          ...
          d3.select(svgRef.current).transition().duration(1500).ease(d3.easeCubicOut)
  ...
  useEffect(() => {
    if (!targetWalletId || !simulationRef.current || !svgRef.current || !zoomBehaviorRef.current) return;
    ...
    if (targetNode) {
        ...
        d3.select(svgRef.current).transition().duration(1000).ease(d3.easeCubicOut)
          .call(zoomBehaviorRef.current.transform, transform);
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full bg-space-900 overflow-hidden rounded-xl border border-space-700 shadow-2xl group" aria-label="Bubble map visualization">
      <style>
        {`
      ...
      {/* --- SETTINGS POPUP --- */}
      {isSettingsOpen && (
          <div className="absolute top-4 left-14 z-30 bg-space-800 rounded-xl border border-space-700 shadow-2xl p-4 w-64 animate-in fade-in slide-in-from-left-2">
              ...
              <div className="space-y-4">
                  <div className="flex items-center justify-between">
                      <span className="text-sm text-white flex items-center gap-2">
                          {showLinks ? <Eye size={14}/> : <EyeOff size={14} className="text-slate-500"/>} Links
                      </span>
                      <button 
                        onClick={() => setShowLinks(!showLinks)}
                        className={`w-10 h-5 rounded-full relative transition-colors ${showLinks ? 'bg-purple-600' : 'bg-space-600'}`}
                      >
                          <span className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${showLinks ? 'left-6' : 'left-1'}`}></span>
                      </button>
                  </div>

                  <div className="flex items-center justify-between">
                      <span className="text-sm text-white flex items-center gap-2">
                          {showLabels ? <Eye size={14}/> : <EyeOff size={14} className="text-slate-500"/>} Labels
                      </span>
                      <button 
                        onClick={() => setShowLabels(!showLabels)}
                        className={`w-10 h-5 rounded-full relative transition-colors ${showLabels ? 'bg-purple-600' : 'bg-space-600'}`}
                      >
                          <span className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-transform ${showLabels ? 'left-6' : 'left-1'}`}></span>
                      </button>
                  </div>
          ...
          <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/70 p-4 animate-in fade-in">
              <div className="bg-space-800 rounded-xl border border-space-700 shadow-2xl max-w-sm w-full overflow-hidden">
                  <div className="p-4 border-b border-space-700 flex justify-between items-center">
                      <h3 className="font-bold text-white flex items-center gap-2"><HelpCircle size={18}/> Interactive Guide</h3>
                      <button onClick={() => setIsHelpOpen(false)} className="text-slate-400 hover:text-white"><X size={18}/></button>
                  </div>
                  <div className="p-4 space-y-4 text-sm text-slate-300">
                      <div className="flex gap-3">
                          <div className="p-2 bg-space-900 border border-space-700 rounded h-fit"><Move size={16}/></div>
                          <div>
                      ...
                      <div className="flex gap-3">
                          <div className="p-2 bg-space-900 border border-space-700 rounded h-fit"><MousePointer2 size={16}/></div>
                          <div>
                              <strong className="text-white block">Inspect Wallet</strong>
                              Click any bubble to view balance, transactions, and get AI insights.
                              <div className="text-xs text-slate-500 mt-1">💡 Use Arrow Keys to cycle through bubbles!</div>
                          </div>
                      </div>
                      <div className="flex gap-3">
                          <div className="p-2 bg-space-900 border border-space-700 rounded h-fit"><ZoomIn size={16}/></div>
                          <div>
              ...
              <div className="absolute inset-0 -z-10" onClick={() => setIsHelpOpen(false)}></div>
          </div>
      )}
      ...
      <div className="absolute inset-x-0 bottom-4 z-20 flex flex-col items-end gap-3 px-3 md:hidden">
        ...
        <div className="flex flex-col gap-2 w-full max-w-[240px]">
          ...
          >
            <div className="flex flex-col gap-2">
              {userNodeFound && (
                <button onClick={handleLocateUser} className="p-2 bg-purple-600 border border-purple-500 text-white rounded-lg shadow-lg transition-all hover:bg-purple-500 hover:border-purple-400" title="Locate My Wallet">
                    <Crosshair size={20} />
                </button>
              )}
              <div className="h-px bg-space-700 w-full my-1"></div>
              <button onClick={handleSnapshot} className="p-2 bg-space-800 border border-space-700 text-slate-300 hover:text-white hover:bg-space-700 hover:border-space-600 rounded-lg shadow-lg transition-all" title="Download Image Snapshot">
                <Camera size={20} className={isSnapshotting ? "text-purple-500" : ""} />
              </button>
              <button onClick={togglePause} className="p-2 bg-space-800 border border-space-700 text-slate-300 hover:text-white hover:bg-space-700 hover:border-space-600 rounded-lg shadow-lg transition-all" title={isPaused ? "Resume Simulation" : "Pause Simulation"}>
                {isPaused ? <Play size={20} /> : <Pause size={20} />}
              </button>
              <button onClick={handleZoomIn} className="p-2 bg-space-800 border border-space-700 text-slate-300 hover:text-white hover:bg-space-700 hover:border-space-600 rounded-lg shadow-lg transition-all" title="Zoom In">
                <ZoomIn size={20} />
              </button>
              <button onClick={handleZoomOut} className="p-2 bg-space-800 border border-space-700 text-slate-300 hover:text-white hover:bg-space-700 hover-border-space-600 rounded-lg shadow-lg transition-all" title="Zoom Out">
                <ZoomOut size={20} />
              </button>
              <button onClick={handleReset} className="p-2 bg-space-800 border border-space-700 text-slate-300 hover:text-white hover:bg-space-700 hover:border-space-600 rounded-lg shadow-lg transition-all" title="Reset View">
                <RotateCcw size={20} />
              </button>
            </div>
          </div>
        ...
        <div className="w-full max-w-[240px]">
          <div className="bg-space-900 rounded-xl border border-space-700 shadow-xl overflow-hidden transition-all duration-300">
              ...
              <div className={`px-4 pb-4 transition-all duration-300 ${isLegendOpen ? 'max-h-[300px] opacity-100 mt-2' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                  ...
                  <div className="flex items-start gap-4">
                      <span className="flex items-center gap-2 text-slate-300 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-rose-400 shadow-[0_0_6px_rgba(251,113,133,0.8)]"></span> Contract</span>
                      <span className="text-slate-500 text-[10px] whitespace-nowrap ml-1">Protocol / Treasury</span>
                  </div>
                  <div className="flex items-start gap-4">
                      <span className="flex items-center gap-2 text-slate-300 whitespace-nowrap"><span className="w-2 h-2 rounded-full bg-white border border-space-600"></span> Node</span>
                      <span className="text-slate-500 text-[10px] whitespace-nowrap ml-1">Interactive Wallet</span>
                  </div>
                  {userNodeFound && (
                      <div className="flex items-center justify-between">
                          <span className="flex items-center gap-2 text-doge-400 font-bold"><span className="w-3 h-3 rounded-full bg-amber-400 border-2 border-white"></span> YOU</span>
      ...
      {/* Legend (desktop) */}
      <div className="hidden md:block absolute bottom-6 left-6 z-20 max-w-[220px]">
        <div className="bg-space-900 rounded-xl border border-space-700 shadow-xl overflow-hidden transition-all duration-300">
            <button
              onClick={() => setIsLegendOpen(!isLegendOpen)}
              className="w-full px-4 py-2 flex items-center justify-between text-[10px] uppercase tracking-widest text-slate-500 font-bold hover:bg-space-800/50"
            ...
            <div className={`px-4 pb-4 transition-all duration-300 ${isLegendOpen ? 'max-h-[300px] opacity-100 mt-2' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                ...
                <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-slate-300"><span className="w-2 h-2 rounded-full bg-rose-400 shadow-[0_0_6px_rgba(251,113,133,0.8)]"></span> Contract</span>
                    <span className="text-slate-500 text-[10px]">Protocol / Treasury</span>
                </div>
                <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-slate-300"><span className="w-2 h-2 rounded-full bg-white border border-space-600"></span> Node</span>
