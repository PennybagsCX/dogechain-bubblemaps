 ...
 *
 * A multi-step modal guide for the Dashboard page.
 * Covers alert management, real-time monitoring, event history, and data management.
 * Triggers on first Dashboard page load or via help icon.
 */

import React, { useEffect, useRef } from "react";
import ReactDOM from "react-dom";
import { X } from "lucide-react";
import { ShieldCheck, Bell, Activity, LineChart, Layers, Download } from "lucide-react";

/**
 * Interface for DashboardGuide props
 */
...
const dashboardGuideSteps = [
  {
    ...
    content:
      "Your central command for monitoring wallet activity. Set up smart alerts, track events in real-time, and never miss important whale movements.",
  ...
  },
  {
    ...
    content:
      "Monitor wallets with three alert types: WALLET (all activity), TOKEN (specific token transfers), and WHALE (large holder movements). Get instant notifications when activity occurs.",
    primaryAction: "Next",
  ...
  },
  {
    ...
    content:
      "Your alerts scan automatically every 30 seconds. Receive browser notifications and in-app alerts with sound when new transactions are detected.",
    primaryAction: "Next",
    secondaryAction: "Skip",
  },
  {
    id: "event-history",
    title: "Event History & Charts",
    icon: <LineChart size={48} className="text-purple-500" />,
    content:
      "View triggered events with full transaction details. Access live price charts from DexScreener to analyze market impact when alerts trigger.",
    primaryAction: "Next",
    secondaryAction: "Skip",
  },
  {
    ...
    content:
      "Track multiple wallets across different alert types. See all your tracked wallets, active alerts, and triggered events in one unified view.",
    primaryAction: "Next",
    secondaryAction: "Skip",
  },
  {
    ...
    content:
      "Export your alert history and triggered events as CSV for backup or analysis. Edit or delete alerts as your monitoring needs evolve.",
...
export const DashboardGuide: React.FC<DashboardGuideProps> = ({
  ...
  const step = dashboardGuideSteps[currentStep];
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      <div
        ...
        aria-describedby="dashboard-guide-content"
          ...
          <button
            ...
            aria-label={
              currentStep === totalSteps - 1
                ? "Complete guide and start using dashboard"
                : "Go to next step"
            }
          >
            {step.primaryAction}
