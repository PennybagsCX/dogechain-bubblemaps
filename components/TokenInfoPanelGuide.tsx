/**
 * TokenInfoPanelGuide Component
 *
 * A multi-step modal guide for the token information panel (left sidebar) of the Map Analysis page.
 * Covers basic features (steps 1-3) and advanced features (steps 4-6).
 * Triggers on first click on holders list or pagination controls.
 */

import React, { useEffect, useRef } from "react";
import ReactDOM from "react-dom";
import { X } from "lucide-react";
import { BarChart3, TrendingUp, ChevronLeft, Trophy, Tag, PieChart } from "lucide-react";

/**
 * Interface for TokenInfoPanelGuide props
 */
export interface TokenInfoPanelGuideProps {
  isOpen: boolean;
  currentStep: number;
  totalSteps: number;
  progress: number;
 ...
 */
const tokenPanelGuideSteps = [
  {
    id: "welcome",
    title: "Welcome to Holder Statistics",
    icon: <BarChart3 size={48} className="text-green-500" />,
    content:
      "Track top token holders in this panel. See who holds the most tokens and their percentage of total supply.",
  ...
  },
  {
    ...
    content:
      "Each row shows: wallet address (masked), token amount, and % of total supply. Click to copy address or view on explorer.",
    primaryAction: "Next",
  ...
  },
  {
    ...
    icon: (
      <div className="flex gap-2 items-center justify-center">
        <ChevronLeft size={48} className="text-purple-500" />
        <ChevronLeft size={48} className="text-purple-500" />
      </div>
    ),
    content:
      "View 10 holders at a time. Use Previous/Next buttons to navigate through all holders. Page number shows your position.",
    primaryAction: "Next",
    secondaryAction: "Skip",
  },
  {
    ...
    content:
      "Holders are ranked by token amount. Rank 1 = largest holder. Watch for 'whale' wallets holding >5% of supply.",
    primaryAction: "Next",
    secondaryAction: "Skip",
  },
  {
    id: "wallet-labels",
    title: "Wallet Labels",
    icon: <Tag size={48} className="text-green-500" />,
    content:
      "Wallets may have labels: 'User' (you), 'Whale' (large holder), 'Exchange' (trading platform). Labels help identify key holders.",
    primaryAction: "Next",
    secondaryAction: "Skip",
  },
  {
    ...
    icon: <PieChart size={48} className="text-emerald-500" />,
    content:
      "Monitor concentration metrics. High concentration = more risk. Look for even distribution across multiple holders.",
 ...
 * TokenInfoPanelGuide Component
 */
export const TokenInfoPanelGuide: React.FC<TokenInfoPanelGuideProps> = ({
  isOpen,
  currentStep,
  totalSteps,
  progress,
  ...
  const step = tokenPanelGuideSteps[currentStep];
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="token-panel-guide-title"
        aria-describedby="token-panel-guide-content"
        ...
        <div className="p-4 sm:p-6 border-b border-space-700">
          ...
          <div className="flex justify-end mb-2">
            <button
              ...
              aria-label="Close token info panel guide"
            >
              <X size={20} />
            </button>
          </div>
        ...
        <div className="px-6 pb-6 flex gap-3 justify-center">
          ...
          {step.secondaryAction && (
            <button
              ...
              aria-label="Skip token info panel guide"
            >
              {step.secondaryAction}
            </button>
          )}
          ...
          <button
            ...
            aria-label={
              currentStep === totalSteps - 1
                ? "Complete guide and start learning"
                : "Go to next step"
            }
          >
            {step.primaryAction}
