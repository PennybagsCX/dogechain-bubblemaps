...
export const WalletSidebar: React.FC<WalletSidebarProps> = (props: WalletSidebarProps) => {
  ...
  useEffect(() => {
    if (wallet) {
      ...
      fetchWalletTransactions(wallet.address, tokenAddress, assetType).then((txs) => {
          ...
          const instruction = createWalletChatSystemInstruction(wallet, txs, tokenName, tokenSymbol, assetType);
          setSystemInstruction(instruction);

          // Add initial greeting
          const labelText = wallet.label ? ` (Identified as ${wallet.label})` : '';
          setMessages([{role: 'model', text: `I've analyzed ${wallet.address.substring(0,6)}...${labelText}. What would you like to know regarding their ${tokenSymbol} activity?`}]);
      });
  ...
  const displayBalance = liveBalance !== null ? liveBalance : wallet.balance;
        ...
        <div className="p-5 bg-space-800 border-b border-space-700 flex items-start justify-between shrink-0 rounded-t-2xl md:rounded-none md:rounded-t-lg">
            <div className="flex-1 min-w-0 mr-4">
              <div className="flex items-center gap-2 mb-1">
                   <h2 id="wallet-details-title" className="text-lg font-bold text-white">Wallet Details</h2>
              ...
              <div className="flex items-center gap-2 mt-1">
                  <p className="text-xs text-slate-400 font-mono" title={wallet.address}>{wallet.address.slice(0, 6)}...{wallet.address.slice(-4)}</p>
                  <button
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 overscroll-contain">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 rounded-lg bg-space-900 border border-space-700">
              <p className="text-[10px] uppercase tracking-wide text-slate-400">{isNFT ? 'Owned NFTs' : 'Balance'}</p>
              <p className={`text-lg font-bold break-words ${isNFT ? 'text-purple-400' : 'text-doge-500'}`}>
          ...
          <div className="flex flex-wrap gap-2">
                {wallet.isWhale && (
                    <span className={`px-2 py-1 text-xs font-bold rounded border border-space-700/20 ${isNFT ? 'text-purple-400 bg-purple-400/10 border-purple-400/30' : 'text-doge-600 bg-doge-600/10 border-doge-600/30'}`}>
                ...
                {onTraceConnections && (
                    ...
                    >
                        {isTracing ? <Loader2 size={10} className="animate-spin"/> : <Network size={10}/>} 
          ...
          {activeTab === 'details' && (
              <div className="animate-fade-in h-[300px] flex flex-col bg-space-900 rounded-xl border border-space-700 overflow-hidden relative isolate">
                  ...
                  <div className="flex-1 overflow-y-auto p-3 space-y-3">
                      {messages.map((msg, i) => (
                          <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-1 ${msg.role === 'user' ? 'bg-space-700' : 'bg-purple-600'}`}>
                                  {msg.role === 'user' ? <User size={12} className="text-slate-300"/> : <BrainCircuit size={12} className="text-white"/>}
                              </div>
                              <div className={`p-2 rounded-lg border border-space-700/20 text-sm max-w-[80%] ${msg.role === 'user' ? 'bg-space-700 text-white' : 'bg-purple-900/30 border-purple-500/30 text-purple-100'}`}>
                                  {msg.text}
                              </div>
                          </div>
                      ))}
                  ...
                  {/* Input Area */}
                  <form onSubmit={handleSendMessage} className="p-2 bg-space-800 border-t border-space-700 flex gap-2">
                      <input
                        type="text"
                        value={input}
                  ...
                  <div className="absolute inset-0 z-[120] flex items-center justify-center bg-space-900/95 backdrop-blur-sm px-4 py-6">
                      <div className="w-full max-w-[240px] mx-auto rounded-xl bg-space-900/90 border border-purple-500/70 px-5 py-5 text-center shadow-2xl flex flex-col items-center gap-2">
                          <p className="text-sm font-bold text-purple-100 leading-tight">FEATURE COMING SOON</p>
                          <p className="text-xs text-slate-200 leading-tight">AI-Powered Wallet Analysis</p>
                      </div>
                  </div>
              </div>
          )}
                  ...
                  <div className="flex-1 overflow-y-auto space-y-3 min-h-0">
                      {isTransactionsLoading ? (
                          ...
                          paginatedTransactions.map((tx) => (
                              <div key={tx.hash} className="p-4 rounded-lg bg-space-900 border border-space-700 flex justify-between items-center gap-2 min-w-0 hover:border-space-600 transition-colors">
                                  <div>
                                      <div className={`text-xs font-bold ${tx.from === wallet.address ? 'text-red-400' : 'text-green-400'}`}>
                                          {tx.from === wallet.address ? 'OUT' : 'IN'}
                                      </div>
                                      <div className="text-xs text-slate-500">{new Date(tx.timestamp).toLocaleDateString()}</div>
                                  </div>
                                  <div className="text-right">
                                      <div className="text-sm font-mono text-white truncate">{tx.value.toLocaleString(undefined, { maximumFractionDigits: isNFT ? 0 : 4 })}</div>
                                      <div className="text-[10px] text-slate-500">{isNFT ? 'NFTs' : (tx.tokenSymbol || tokenSymbol)}</div>
                                  </div>
                              </div>
                          ))
                      )}
                  ...
                  {filteredTransactions.length > pageSize && (
                      <div className="mt-3 shrink-0">
                          {paginationMode === 'load-more' && totalPages < 5 ? (
                              ...
                              >
                                  {currentPage >= totalPages ? 'All transactions loaded' : `Load More (Page ${currentPage + 1})`}
                              </button>
                          ) : (
