...
import { Token, Wallet, Transaction, Link, AssetType, ScanProgressUpdate, ScanMetadata } from '../types';
...
export const detectContractType = async (address: string): Promise<AssetType | null> => {
    try {
        ...
        try {
            const metaRes = await fetchSafe(`${EXPLORER_API_V1}?module=token&action=getToken&contractaddress=${address}`);
            if (metaRes.ok) {
                ...
                if (metaJson.status === "1" && metaJson.result) {
                    ...
                    if (rawType?.includes('721') || rawType?.includes('1155') || rawType === 'erc721' || rawType === 'erc1155') {
        ...
        try {
            const abiRes = await fetchSafe(`${EXPLORER_API_V1}?module=contract&action=getsourcecode&address=${address}`);
            if (abiRes.ok) {
                ...
                if (abiStr && abiStr !== 'Contract source code not verified') {
                    ...
                    if (Array.isArray(abi)) {
                        const hasOwnerOf = abi.some((item: any) => item?.type === 'function' && item?.name === 'ownerOf');
                        const hasTokenURI = abi.some((item: any) => item?.type === 'function' && item?.name === 'tokenURI');
                        const hasSupportsInterface = abi.some((item: any) => item?.type === 'function' && item?.name === 'supportsInterface');
                        const hasSafeTransfer721 = abi.some((item: any) => item?.type === 'function' && item?.name === 'safeTransferFrom' && item?.inputs?.length === 3);
                        const hasSafeTransfer1155 = abi.some((item: any) => item?.type === 'function' && item?.name === 'safeTransferFrom' && item?.inputs?.length === 5);
                        if (hasOwnerOf || hasTokenURI || hasSupportsInterface || hasSafeTransfer721 || hasSafeTransfer1155) {
                            return AssetType.NFT;
                        }
                        const hasDecimals = abi.some((item: any) => item?.type === 'function' && item?.name === 'decimals');
        ...
        const nftRes = await fetchSafe(`${EXPLORER_API_V1}?module=account&action=tokennfttx&contractaddress=${address}&page=1&offset=1&sort=desc`);
        if (nftRes.ok) {
        ...
        const ftRes = await fetchSafe(`${EXPLORER_API_V1}?module=account&action=tokentx&contractaddress=${address}&page=1&offset=1&sort=desc`);
        if (ftRes.ok) {
...
const BOOTSTRAP_TOKENS: Record<string, { decimals: number, symbol?: string, name?: string, type?: AssetType }> = {
    '0xe3f5a90f9cb311505cd691a46596599aa1a0ad7d': { decimals: 6, symbol: 'USDT', name: 'Tether USD', type: AssetType.TOKEN },
    '0xb7ddc6414bf4f5515b52d8bdd69973ae205ff101': { decimals: 18, symbol: 'wDOGE', name: 'Wrapped DOGE', type: AssetType.TOKEN },
    '0x7b4328c127b85369d9f82ca0503b000d09cf9180': { decimals: 18, symbol: 'DC', name: 'Dogechain', type: AssetType.TOKEN },
    '0xd38b22794b308a2e55808a13d1e6a80c4be94fd5': { decimals: 0, symbol: 'DPunks', name: 'RealDogePunks', type: AssetType.NFT },
};
...
const getDecimals = (address: string, apiDecimals: string | number | null | undefined, type: AssetType): number => {
    ...
    // 3. API provided
    if (apiDecimals !== null && apiDecimals !== undefined && apiDecimals !== "null" && apiDecimals !== "") {
...
const fetchMetadataFromTokenList = async (address: string): Promise<{name?: string, symbol?: string, decimals?: number} | null> => {
    try {
        const res = await fetchSafe(`${EXPLORER_API_V1}?module=token&action=getToken&contractaddress=${address}`);
        if (res.ok) {
            const data = await res.json();
            if (data.status === "1" && data.result) {
                ...
                return {
                    ...
                    decimals: tokenInfo.tokenDivisor || tokenInfo.decimals ? parseInt(tokenInfo.tokenDivisor || tokenInfo.decimals) : undefined
                };
            }
        }
        return null;
...
export const fetchTokenData = async (address: string, preferredType: AssetType = AssetType.TOKEN): Promise<Token | null> => {
  ...
  if (cached && (cached.symbol === 'TOKEN' || cached.symbol === '???' || cached.name === 'Unverified Token' || cached.name === 'Unknown Token')) {
  ...
  try {
    ...
    let type = detectedType || cached?.type || bootstrap?.type || preferredType;
    ...
    finalName = finalName || (type === AssetType.NFT ? 'Unverified Collection' : 'Unverified Token');
    ...
    if (finalName && finalSymbol && finalName !== "Unverified Token") {
        saveMetadataToCache(cleanAddress, { symbol: finalSymbol, name: finalName, decimals: finalDecimals, type });
    }

    // Try supply via V1 stats (cheap) — skip if it fails
    let totalSupply = 0;
    try {
      const v1Res = await fetchSafe(`${EXPLORER_API_V1}?module=stats&action=tokensupply&contractaddress=${cleanAddress}`);
...
export const fetchTokenHolders = async (token: Token): Promise<{ wallets: Wallet[], links: Link[] }> => {
  const cleanAddress = validateTokenAddress(token.address);
  const decimals = token.decimals !== undefined ? token.decimals : (token.type === AssetType.NFT ? 0 : 18);

  // V1-only: avoid V2 holders endpoint (returns 400 in production)
  try {
    await sleep(150); // small spacing to be gentle on API
    const v1Response = await fetchSafe(`${EXPLORER_API_V1}?module=token&action=getTokenHolders&contractaddress=${cleanAddress}&page=1&offset=100`);
    ...
    const holders: { address: string, balance: string, is_contract: boolean }[] = v1Json.result.map((item: any) => ({
    ...
    let processedWallets: Wallet[] = holders.map((h) => {
        ...
        try {
          ...
          if (data.result && Array.isArray(data.result)) {
              data.result.forEach((tx: any) => {
                  ...
                  const match = processedWallets.find(w => w.address.toLowerCase() === other.toLowerCase());
                  
                  if (match && match.id !== whale.id) {
                      const linkExists = links.some(l => 
                          (typeof l.source === 'string' ? l.source === whale.id : l.source.id === whale.id) && 
...
export const fetchWalletTransactions = async (walletAddress: string, tokenAddress?: string, type: AssetType = AssetType.TOKEN): Promise<Transaction[]> => {
  try {
      ...
      for (let i = 0; i < offsets.length; i++) {
          ...
          console.log(`[fetchWalletTransactions] Attempting offset ${offset} for ${cleanWallet.slice(0, 8)}...`);

          const transactions = await fetchWithOffset(offset);

          // If we found transactions, return them immediately
          if (transactions.length > 0) {
              console.log(`[fetchWalletTransactions] Found ${transactions.length} transactions with offset ${offset}`);
              return transactions;
          }
      ...
      console.log(`[fetchWalletTransactions] No transactions found for ${cleanWallet.slice(0, 8)}... after trying all offsets`);
      return [];
...
export const fetchTokenBalance = async (walletAddress: string, tokenAddress: string, knownDecimals?: number): Promise<number> => {
    try {
        ...
        const data = await res.json();
        if (data.status === "1" && data.result) {
             const decimals = knownDecimals !== undefined ? knownDecimals : getDecimals(tokenAddress, null, AssetType.TOKEN); 
...
export const scanWhaleContracts = async (
    ...
    for (const whale of WHALE_ADDRESSES) {
        try {
            ...
            console.log(`[Whale Scan] Whale ${whale}: found ${discoveredContracts.size} contracts so far`);
        } catch (e) {
...
export const fetchWalletAssets = async (walletAddress: string): Promise<{ tokens: Token[]; nfts: Token[] }> => {
    try {
        const cleanWallet = validateWalletAddress(walletAddress);
        ...
        const recordAsset = (contract: string | undefined, symbol: string | undefined, name: string | undefined, decimals: number | undefined, type: AssetType) => {
            if (!contract) return;
        ...
        const fetchTransfersV1 = async (action: 'tokentx' | 'tokennfttx', offsets: number[]) => {
            for (const offset of offsets) {
                try {
                    const res = await fetchSafe(`${EXPLORER_API_V1}?module=account&action=${action}&address=${cleanWallet}&page=1&offset=${offset}&sort=desc`);
                    if (!res.ok) continue;
                    const json = await res.json();
                    if (json.status === "1" && Array.isArray(json.result)) {
        ...
        ftTransfers.forEach((tx: any) => {
            ...
            const decimals = tx.tokenDecimal ? parseInt(tx.tokenDecimal) : tx.token?.decimals ? parseInt(tx.token.decimals) : undefined;
        ...
        if (nftTransfersV2.length === 0 && nftBalancesV2.length === 0) {
            ...
            for (const off of smallOffsets) {
                try {
                    ...
                    v1.forEach((tx: any) => {
                        recordAsset(tx.contractAddress || tx.contract, tx.tokenSymbol, tx.tokenName, 0, AssetType.NFT);
                    });
...
export const fetchWalletAssetsHybrid = async (
    ...
    try {
        ...
        const triggerProgress = (phase: ScanProgressUpdate['phase'], progress: number, operation: string) => {
            if (onProgress) {
                onProgress({
                    phase,
                    progress,
        ...
        const fetchV2Pages = async (endpoint: string, startPage: number, maxPages: number, delayMs: number) => {
            const results: any[] = [];
        ...
        const fetchV1Offsets = async (action: string, offsets: number[], delayMs: number) => {
            ...
            for (const offset of offsets) {
                try {
                    await sleep(delayMs);
                    totalRequests++;
                    const res = await scheduler.schedule(() => fetchSafe(`${EXPLORER_API_V1}?module=account&action=${action}&address=${cleanWallet}&page=1&offset=${offset}&sort=desc`));
                    if (!res.ok) continue;
                    const json = await res.json();
                    if (json.status === "1" && Array.isArray(json.result)) {
        ...
        const quickV2Transfers = await fetchV2Pages(`/addresses/${cleanWallet}/token-transfers`, 1, 3, 1000);
        quickV2Transfers.forEach((tx: any) => {
            ...
            const type = (tx.token?.type?.toString().toLowerCase().includes('721') ||
                         tx.token?.type?.toString().toLowerCase().includes('1155'))
                        ? AssetType.NFT : AssetType.TOKEN;
            recordAsset(address, symbol, name, decimals, type);
        });

        // V2 token balances (pages 1-3)
        const quickV2Balances = await fetchV2Pages(`/addresses/${cleanWallet}/token-balances`, 1, 3, 1000);
        quickV2Balances.forEach((entry: any) => {
            const address = entry.contractAddress || entry.contract || entry.token?.address;
            const symbol = entry.tokenSymbol || entry.token?.symbol;
            const name = entry.tokenName || entry.token?.name;
            const type = (entry.token?.type?.toString().toLowerCase().includes('721') ||
        ...
        try {
            ...
            const v1NftProbe = await scheduler.schedule(() =>
                fetchSafe(`${EXPLORER_API_V1}?module=account&action=tokennfttx&address=${cleanWallet}&page=1&offset=20&sort=desc`)
            );
            if (v1NftProbe.ok) {
        ...
        triggerProgress('quick', 15, `Quick scan complete: ${tokensMap.size} tokens, ${nftsMap.size} NFTs found`);
        ...
        const deepV2Transfers = await fetchV2Pages(`/addresses/${cleanWallet}/token-transfers`, 4, 20, 2000);
        ...
        const deepV2Balances = await fetchV2Pages(`/addresses/${cleanWallet}/token-balances`, 4, 20, 2000);
        ...
        triggerProgress('deep-v2', 40, `Deep V2 scan complete: ${tokensMap.size} tokens, ${nftsMap.size} NFTs found`);
        ...
        console.log(`[Phase 3] Searching for all transfers with ${allNftOffsets.length} different offsets`);
        ...
        try {
            ...
            for (let i = 0; i < contractsArray.length; i += 5) {
                ...
                triggerProgress('deep-v1', Math.floor(progress), `Detecting contract types... ${Math.floor((i + batch.length) / contractsArray.length * 100)}%`);
            }
        ...
        triggerProgress('deep-v1', 65, `Deep V1 scan complete: ${tokensMap.size} tokens, ${nftsMap.size} NFTs found`);
        ...
        const allContracts = Array.from(new Set([
            ...Array.from(tokensMap.keys()),
            ...Array.from(nftsMap.keys())
        ]));
        ...
        for (let i = 0; i < allContracts.length; i += 10) {
            ...
            for (const contract of batch) {
                try {
                    ...
                    const res = await scheduler.schedule(() =>
                        fetchSafe(`${EXPLORER_API_V1}?module=account&action=tokenbalance&contractaddress=${contract}&address=${cleanWallet}`)
                    );
                    const json = await res.json();
            ...
            triggerProgress('balance-check', Math.floor(progress), `Verifying balances... ${Math.floor((i + batch.length) / allContracts.length * 100)}%`);
        }
        ...
        try {
            ...
            const needsRefresh = discoveredContracts.length === 0 ||
            ...
            if (needsRefresh) {
                ...
                for (let i = 0; i < contractsArray.length; i += 10) {
                    ...
                    triggerProgress('whale-scan', Math.floor(progress), `Processing whale contracts... ${Math.floor((i + batch.length) / contractsArray.length * 100)}%`);
                }
            ...
            for (let i = 0; i < discoveredContracts.length; i += 20) {
                ...
                for (const dbContract of batch) {
                    try {
                        ...
                        if (hasBalance) {
                            ...
                            recordAsset(contract, dbContract.symbol, dbContract.name, type === AssetType.NFT ? 0 : 18, type);
                            newAssetsFound++;
                ...
                triggerProgress('whale-scan', Math.floor(progress), `Checking discovered contracts... ${Math.floor((i + batch.length) / discoveredContracts.length * 100)}%`);
            }
        ...
        triggerProgress('balance-check', 100, `Complete: ${finalTokens.length} tokens, ${finalNfts.length} NFTs in ${Math.floor(duration / 1000)}s`);

        console.log(`[Complete] Found ${finalTokens.length} tokens, ${finalNfts.length} NFTs in ${Math.floor(duration / 1000)}s (${totalRequests} requests)`);
...
export const findInteractions = async (targetWallet: Wallet, allWallets: Wallet[], tokenAddress: string): Promise<Link[]> => {
    try {
        ...
        if (data.result && Array.isArray(data.result)) {
            data.result.forEach((tx: any) => {
                const other = tx.from.toLowerCase() === targetWallet.address.toLowerCase() ? tx.to : tx.from;
