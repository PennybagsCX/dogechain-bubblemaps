...
export const generateTokenSummary = async (
  ...
  assetType: AssetType = AssetType.TOKEN
    ...
    const infraHolders = topHolders.filter(w => INFRASTRUCTURE_ADDRESSES.includes(w.address.toLowerCase()));

    // 2. Separate Contracts (Likely LP/DEX/Staking) from Real User Whales
    const contracts = topHolders.filter(w => w.isContract && !INFRASTRUCTURE_ADDRESSES.includes(w.address.toLowerCase()));
    const realWhales = topHolders.filter(w => !w.isContract && !INFRASTRUCTURE_ADDRESSES.includes(w.address.toLowerCase()));
    ...
    try {
        ...
        const response = await ai.models.generateContent({
            ...
            contents: prompt,
            config: {
                systemInstruction: "You are a blockchain risk analyst. You differentiate between locked/burned supply, liquidity pools, and dangerous user whales."
            }
        });
